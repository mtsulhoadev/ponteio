...
// Define todos os cartões — coluna esquerda e direita (igual ao formulário físico)
$cartoes_esquerda = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Calcula totais de cada coluna
    $total_cartoes_esq = 0;
    foreach (array_keys($cartoes_esquerda) as $k) {
        $total_cartoes_esq += floatval(str_replace(',', '.', $dados_post[$k] ?? '0'));
    }

    $total_cartoes_dir = 0;
    foreach (array_keys($cartoes_direita) as $k) {
        $total_cartoes_dir += floatval(str_replace(',', '.', $dados_post[$k] ?? '0'));
    }

    $dinheiro        = floatval(str_replace(',', '.', $dados_post['dinheiro'] ?? '0'));
    $nota_a_prazo    = floatval(str_replace(',', '.', $dados_post['nota_a_prazo'] ?? '0'));
    $despesas        = floatval(str_replace(',', '.', $dados_post['despesas'] ?? '0'));
    $total_depositado= floatval(str_replace(',', '.', $dados_post['total_depositado'] ?? '0'));

    $total_pedido    = $dinheiro + $total_cartoes_esq + $total_cartoes_dir + $nota_a_prazo + $despesas;
    $diferenca       = $total_pedido - $total_depositado;

    // Aqui você salvaria no banco de dados
    // INSERT INTO mapas_caixa (usuario_id, posto_id, data, dinheiro, ..., total_pedido, total_depositado, diferenca)
    ...
    $dados = [
        ...
        'diferenca'       => $diferenca,
        'dinheiro'        => $dinheiro,
        'total_cartoes'   => $total_cartoes_esq + $total_cartoes_dir,
    ];
}
?>
<!DOCTYPE html>
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .st-value.verde { color: #4ade80; }
  .st-value.vermelho { color: #ff6b7a; }
  .st-value.amarelo { color: #ff8c00; }
  ...
  .form-title { font-size: 16px; font-weight: 700; color: #e8eef4; display: flex; align-items: center; gap: 8px; }
  ...
  @media (max-width: 900px) {
    .colunas-grid { grid-template-columns: 1fr; }
  }
  ...
  .btn-secondary:hover { color: #e8eef4; background: rgba(255,255,255,0.08); }

  /* Total ao vivo no topo da seção */
  .total-live-badge {
...
<div class="sucesso-banner">
  ...
  <div>
    <div class="sucesso-title">Mapa de caixa enviado com sucesso!</div>
    <div class="sucesso-meta"><?= htmlspecialchars($nome) ?> · <?= htmlspecialchars($posto) ?> · <?= $hojeF ?></div>
    <div class="sucesso-totais">
      <div class="st-item">
        <div class="st-label">Dinheiro</div>
        <div class="st-value">R$ <?= number_format($dados['dinheiro'], 2, ',', '.') ?></div>
      </div>
      <div class="st-item">
        <div class="st-label">Total cartões</div>
        <div class="st-value">R$ <?= number_format($dados['total_cartoes'], 2, ',', '.') ?></div>
      </div>
      <div class="st-item">
        <div class="st-label">Total pedido no caixa</div>
        <div class="st-value amarelo">R$ <?= number_format($dados['total_pedido'], 2, ',', '.') ?></div>
      </div>
      <div class="st-item">
        <div class="st-label">Total depositado</div>
        <div class="st-value">R$ <?= number_format($dados['total_depositado'], 2, ',', '.') ?></div>
      </div>
      <div class="st-item">
        <div class="st-label">Diferença (+/-)</div>
        <div class="st-value <?= $dados['diferenca'] == 0 ? 'verde' : ($dados['diferenca'] > 0 ? 'verde' : 'vermelho') ?>">
          R$ <?= number_format($dados['diferenca'], 2, ',', '.') ?>
        </div>
      </div>
    </div>
...
<div class="form-card">
  ...
  <div class="form-header">
    ...
    <div class="meta-fields">
      <div class="meta-field">
        <label>Posto</label>
        <input type="text" name="posto" value="<?= htmlspecialchars($posto) ?>" readonly>
      </div>
      <div class="meta-field">
        <label>Funcionário</label>
        <input type="text" name="funcionario" value="<?= htmlspecialchars($nome) ?>" readonly>
      </div>
      <div class="meta-field">
        <label>Data</label>
  ...
  <div class="form-body">
    ...
    <!-- GRID DUPLA DE CARTÕES -->
    <div class="colunas-grid">

      <!-- COLUNA ESQUERDA -->
      <div>
        <div class="coluna-header">
          <span>Cartões</span><span style="text-align:right">Total (R$)</span>
        </div>
        ...
        <div class="row-cartao">
          ...
          <input
            type="text"
            class="input-valor campo-cartao"
            name="<?= $key ?>"
            placeholder="0,00"
            inputmode="decimal"
            oninput="recalcular()"
        ...
        <div class="linha-total">
          <span class="lt-label">Subtotal coluna</span>
          <span class="lt-value" id="subtotal_esq">R$ 0,00</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="linha-total">
          <span class="lt-label">Subtotal coluna</span>
          <span class="lt-value" id="subtotal_dir">R$ 0,00</span>
        </div>
      </div>
    ...
    <!-- DINHEIRO + NOTAS A PRAZO + DESPESAS -->
    <div class="bottom-section">
      <div class="campo-destaque">
        <label>💵 Dinheiro</label>
        <input type="text" name="dinheiro" id="dinheiro" placeholder="0,00" inputmode="decimal" oninput="recalcular()">
      </div>
      <div class="campo-destaque">
        <label>📄 Nota a Prazo</label>
        <input type="text" name="nota_a_prazo" id="nota_a_prazo" placeholder="0,00" inputmode="decimal" oninput="recalcular()">
      </div>
      <div class="campo-destaque">
        <label>🧾 Despesas</label>
        <input type="text" name="despesas" id="despesas" placeholder="0,00" inputmode="decimal" oninput="recalcular()">
      </div>
      <div class="campo-destaque">
        <label>🏦 Total Depositado</label>
        <input type="text" name="total_depositado" id="total_depositado" placeholder="0,00" inputmode="decimal" oninput="recalcular()">
      </div>
    </div>
    ...
    <div class="totais-gerais">
      ...
      <div class="tg-grid">
        <div class="tg-item">
          <div class="tg-label">Total pedido no caixa</div>
          <div class="tg-value total" id="res_total_pedido">R$ 0,00</div>
        </div>
        <div class="tg-item">
          <div class="tg-label">Total depositado</div>
          <div class="tg-value" id="res_total_dep">R$ 0,00</div>
        </div>
        <div class="tg-item">
          <div class="tg-label">Diferença (+/-)</div>
          <div class="tg-value diferenca-zero" id="res_diferenca">R$ 0,00</div>
        </div>
      </div>
    </div>
    ...
    <div class="assinatura-box">
      <div class="as-text">
        Declaro que o meu caixa foi conferido e estou ciente que posso ser punido por qualquer irregularidade,
        conforme o Regulamento Interno de Trabalho.
      </div>
      <div class="as-campo">
        Assinatura digital confirmada pelo login do sistema
      </div>
    </div>
...
function recalcular() {
    ...
    esqKeys.forEach(k => {
        ...
        if (el) totalEsq += parseBR(el.value);
    });
    ...
    dirKeys.forEach(k => {
        const el = document.querySelector(`input[name="${k}"]`);
        if (el) totalDir += parseBR(el.value);
    });
    ...
    // Atualiza subtotais das colunas
...
// Formata campos ao sair (blur)
document.addEventListener('blur', function(e) {
    if (e.target.classList.contains('input-valor') ||
        ['dinheiro','nota_a_prazo','despesas','total_depositado'].includes(e.target.id)) {
